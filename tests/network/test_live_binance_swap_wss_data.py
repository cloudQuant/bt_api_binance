...
_SWAP_REQUEST_SPEC = importlib.util.spec_from_file_location(
    "test_live_binance_swap_request_data",
    Path(__file__).resolve().with_name("test_live_binance_swap_request_data.py"),
)
_SWAP_REQUEST_MODULE = importlib.util.module_from_spec(_SWAP_REQUEST_SPEC)
assert _SWAP_REQUEST_SPEC is not None
assert _SWAP_REQUEST_SPEC.loader is not None
...
def test_binance_wss_data_feed():
    ...
    if not (
        ...
        and receive_binance_funding_rate_data
        and receive_binance_mark_price_data
        and receive_binance_agg_trade_data
    ):
...
def test_binance_wss_mini_ticker():
    ...
    if len(items) == 0:
        pytest.skip("Skipped (no data, likely network): mini_ticker received no data")
...
def test_binance_wss_all_mini_ticker():
    ...
    items = _collect_wss_data(data_queue, wait_seconds=10)
    if len(items) == 0:
        pytest.skip("Skipped (no data, likely network): all_mini_ticker received no data")
...
def test_binance_wss_all_book_ticker():
    ...
    if not received_ticker:
        pytest.skip("Skipped (no data, likely network): all_book_ticker returned no ticker data")
...
def test_binance_wss_continuous_kline():
    ...
    if not received_bar:
        pytest.skip("Skipped (no data, likely network): continuous_kline returned no bar data")
...
def test_binance_wss_all_mark_price():
    ...
    if not received_mark:
        pytest.skip("Skipped (no data, likely network): all_mark_price returned no mark price data")
...
def test_binance_wss_all_ticker():
    ...
    items = _collect_wss_data(data_queue, wait_seconds=5)
    if len(items) == 0:
        pytest.skip("Skipped (no data, likely network): all_ticker received no data")
